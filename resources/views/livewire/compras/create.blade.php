<form wire:submit.prevent="save">
        ...
        <div class="row">
            <div class="col-12 col-lg-4">
                ...
                <div class="card">
                    <div class="card-header">Cabecera de la compra</div>
                    <div class="card-body">

                        <div class="form-floating mb-3">
                            <select class="form-control" wire:model="compra.personas_id">
                                <option selected disabled value="null">--Seleccione el Proveedor--</option>
                                @foreach($proveedores as $pro)
                                    <option value="{{ $pro->id }}"> {{$pro->nombre}} </option>
                                @endforeach
                            </select>
                            <label>Proveedor de la compra</label>
                            @error('compra.personas_id')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>

                        <div class="form-floating mb-3">
                            <select class="form-control" wire:model="compra.aperturas_id" wire:change="selectCaja">
                            ...
                            <label>Caja para el desembolso</label>
                            @error('compra.aperturas_id')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>

                        <div class="form-floating mb-3">
                            <input type="number" class="form-control" placeholder="Nª factura" wire:model="compra.numero_factura"/>
                            <label>Nª factura</label>
                            ...
                            <tbody>
                                @foreach($arrDetalle as $index => $puntosResult)
                                    ...
                                    <tr>
                                        <td>
                                            <div class="form-floating mb-3">
                                                <select class="form-control" wire:model="arrDetalle.{{$index}}.productos_id">
